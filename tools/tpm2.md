# Криптографический процессор TPM
   ...
   * [Управление сессиями и политиками](#управление-сессиями-и-политиками)
      * [Управление сессиями](#управление-сессиями)
      * [Создание и управление политиками](#создание-и-управление-политиками)
      * [Условия политик](#условия-политик)
      * [Авторизация по политике](#авторизация-по-политике)
...
### Управление сессиями
<pre>
<b>tpm2 startauthsession</b>           Начинает сессию авторизации с TPM
<b>tpm2 sessionconfig</b>              Конфигурирует атрибуты сессии, такие как симметричный алгоритм или использование аудита
</pre>
### Создание и управление политиками
<pre>
<b>tpm2 createpolicy</b>               Начинает сессию политики или связывает хэш политики с объектом
<b>tpm2 getpolicydigest</b>            Вычисляет хэш текущей сессии политики
<b>tpm2 policyrestart</b>              Перезапускает текущую сессию политики, очищая все ранее установленные условия
<b>tpm2 setprimarypolicy</b>           Устанавливает политику авторизации для иерархии, которая будет унаследована первичными ключами
</pre>
### Условия политик
<pre>
<b>tpm2 policycommandcode</b>          Добавляет в политику условие, ограничивающее её использование только для указанной TPM команды
<b>tpm2 policycountertimer</b>         Добавляет в политику условие, связанное со значениями счетчика (TPM_COUNTER) или времени (TPM_TIME)
<b>tpm2 policycphash</b>               Добавляет в политику условие, основанное на хэше параметров команды (cpHash)
<b>tpm2 policyduplicationselect</b>    Добавляет в политику условие, связанное с командой TPM2_Duplicate
<b>tpm2 policylocality</b>             Добавляет в политику условие, ограничивающее её использование определенной локальностью
<b>tpm2 policynamehash</b>             Добавляет в политику условие, основанное на хэше имени объекта
<b>tpm2 policynv</b>                   Добавляет в политику условие, основанное на содержимом NV-индекса и операции сравнения
<b>tpm2 policynvwritten</b>            Добавляет в политику условие, проверяющее, был ли записан определенный NV-индекс
<b>tpm2 policyor</b>                   Добавляет в политику условие "ИЛИ", позволяя удовлетворить политику одним из нескольких указанных дайджестов политик
<b>tpm2 policypcr</b>                  Добавляет в политику условие, основанное на текущих значениях PCR
<b>tpm2 policytemplate</b>             Добавляет в политику условие, проверяющее совпадение дайджеста шаблона объекта с предоставленным
</pre>
### Авторизация по политике
<pre>
<b>tpm2 policyauthvalue</b>            Удовлетворяет условие политики, требующее знания authValue (например, пустого пароля)
<b>tpm2 policypassword</b>             Удовлетворяет условие политики, требующее знания пароля (authValue)
<b>tpm2 policyauthorize</b>            Удовлетворяет условие политики, требующее подписи от доверенного ключа
<b>tpm2 policysigned</b>               Удовлетворяет условие политики, требующее подписи от доверенного ключа (аналогично policyauthorize)
<b>tpm2 policyauthorizenv</b>          Удовлетворяет условие политики, сравнивая его с содержимым NV-индекса
<b>tpm2 policysecret</b>               Удовлетворяет условие политики, используя общий секрет, полученный, например, через HMAC или ECDH
<b>tpm2 policyticket</b>               Удовлетворяет условие политики путем проверки тикета, ранее выданного TPM (например, тикета аттестации)
</pre>
